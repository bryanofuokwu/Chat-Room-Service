...
char* cread(int sockfd){
    char *recv_buffer = NULL;  /* declare a pointer, and initialize to NULL */
    recv_buffer = malloc (MAX_DATA * sizeof *recv_buffer); /* allocate a 5-byte block of memory for name */
    recv(sockfd, recv_buffer, MAX_DATA ,0);
    return recv_buffer;
}

int cwrite(char * _msg, int _size, int sockfd){
    int len = send(sockfd, _msg, _size +1 ,0);
    return len;
}
